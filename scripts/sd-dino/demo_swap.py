...
def compute_pair_feature(model, aug, save_path, files, category, mask=False, dist='cos', real_size=960):
    ...
    # removed code here for model loading and put it externally
...
def process_images(src_img_path,trg_img_path,categories):
    ...
    # make directory save_path if not exists
...
def demo_swap(src_input_path, src_input_ext, trg_input_path, start, end, categories):

    src_paths=[]
    for i in range(start, end):
        zero_padding = "0" * (i < 10)
        src_paths.append(src_input_path + "/0" + zero_padding + str(i) + src_input_ext,)
    
    for src_path in src_paths:
        print("Processing image: ", src_path)
        src_img_path = src_path
        
        #result = process_images(src_img_path, trg_img_path, categories)
        result = process_images(trg_input_path, src_img_path, categories)
